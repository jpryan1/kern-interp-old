...
double laplace_error3d(const ki_Mat& domain,
  ...
  for (int i = 0; i < domain_points.size(); i += 3) {
    ...
    if(bc == BoundaryCondition::ELECTRON_3D){
      potential = -1.0/(4.0*M_PI*sqrt(pow(x0 + 3, 2) + pow(x1 + 2, 2) + pow(x2 + 2, 2)));
...
TEST(IeSolverTest, StokesSphereAnalyticAgreement) {
  ...
  boundary3d->initialize(96,  BoundaryCondition::STOKES_3D_MIX);
  ...
  get_domain_points3d(domain_size, &domain_points3d, 0.25,1);
